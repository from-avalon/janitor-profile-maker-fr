...
cannot provide, because JanitorAI does not render them for you. frame.js falls
back to rebuilding those two from JanitorAI's own class names when this file
does not carry them.
...
Run:  python tools/extract_fragments.py [path-to.mhtml]
"""
...
def main():
    ...
    print("  %d/%d emotion blocks -> preview/vendor/emotion-extra.css (%d KB)"
          % (len(kept), len(available), len(extra) // 1024))
    if dropped_theme:
